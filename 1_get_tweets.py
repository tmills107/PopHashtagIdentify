"""
Data Collection Part 1 (Spring 2022)

Queries Twitter for most recent 50 tweets containing #BlackTwitter.
...
# Combine pages into a list that we will iterate through below. In this script since it is a limited pull there is only one page
all_pages = [tweets] 

# Create list that will collect the dfs for each page; will each be concatenated at the end into a master df.
page_tweet_dfs = []
page_media_dfs = []
    ...
    for item in page['data']:
      ...
      # Creates a variable hashtags and is filled with the "tag" data in each case. If there are no hashtags it is left empty. This is done because tweepy could not do this on its own. 
        if "hashtags" in item["entities"]:
            #hashtags = [d["tag"] for d in item["entities"]["hashtags"]]
...
all_pages_tweets_df.to_csv(file_name_prefix + 'tweets.csv', index=False)
all_pages_media_df.to_csv(file_name_prefix + 'media.csv')
